/* =========================================================================
 * Lesson social card — /academy/[slug]. Unknown slugs fall back to the hub
 * framing so a stale share link still renders a branded card.
 * Hex values mirror packages/ui/tokens.css.
 * ======================================================================= */

import { ImageResponse } from "next/og";
import { tierLabel } from "@/lib/academy/access";
import { findLesson, findLevel } from "@/lib/academy/curriculum";

export const alt = "HeroPips Academy lesson";
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const level = lesson ? findLevel(lesson.level) : null;

  const eyebrow = lesson
    ? `LESSON ${String(lesson.number).padStart(2, "0")} — LEVEL ${level?.number ?? 0}`
    : "HEROPIPS ACADEMY";
  const title = lesson ? lesson.title : "Curious trader to confident trader.";
  const titleSize = title.length > 34 ? 58 : title.length > 22 ? 70 : 84;
  const chips = lesson
    ? [`+${lesson.xp} XP`, `${lesson.minutes} MIN`, tierLabel(lesson.access).toUpperCase()]
    : ["18 LESSONS", "XP + STREAKS", "VERIFIED CERTIFICATES"];
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ display: "flex", fontSize: 24, letterSpacing: 5, color: MID }}>HEROPIPS</div>
        </div>

        {/* lesson framing */}
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <div style={{ display: "flex", fontSize: 26, letterSpacing: 6, color: VOLT }}>{eyebrow}</div>
          <div
            style={{
              display: "flex",
              fontSize: titleSize,
