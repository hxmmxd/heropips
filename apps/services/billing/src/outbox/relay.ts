  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      for (const row of rows) {
        const payload = row.payload.payload as { order_id?: string } | undefined;
        await this.producer.send({
          topic: row.topic,
          messages: [
            {
              key: payload?.order_id ?? row.payload.event_id,
      ...
      if (now - this.lastWarnAt >= WARN_EVERY_MS) {
        ...
        console.warn(
          `[billing] outbox relay: Kafka unavailable, will keep retrying (${(err as Error).message})`,
        );
      }
    } finally {
      this.ticking = false;
