  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        setMessage('A one-time verification code (OTP) has been sent to your email.');
      } else {
    ...
    <div className={`auth-page ${theme === 'light' ? 'auth-light' : ''}`}>
      ...
      <div className="auth-card">
        ...
        <h1 className="auth-heading">
          {showOtpScreen ? 'Verify Email Address' : isSignUp ? 'Create your account' : 'Welcome back'}
        </h1>
        ...
        {showOtpScreen ? (
          <form onSubmit={handleOtpVerify} className="auth-form">
            <div className="auth-input-group">
              <label htmlFor="otpCode">Verification Code (6-Digit OTP)</label>
              <input
                id="otpCode"
                type="text"
                maxLength={6}
            ...
            <button
              type="button"
              className="auth-btn"
              style={{ background: 'transparent', border: '1px solid var(--border)', marginTop: '8px', color: 'var(--text)' }}
              onClick={() => {
                setShowOtpScreen(false);
                setError('');
                setMessage('');
